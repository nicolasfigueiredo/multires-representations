...
def lin_comb_mult_res(x, sr, win_lengths, FChT_flags, asym_flags, C, alphas, hop_length, N_alphas, C_limits, center=True):
    ...
    STFT_tensor = spectrogram_tensor_prep(x, sr, win_lengths, hop_length, window='hann', center=center, interpolate=True, interp_kind='linear')
        ...
        if FChT_flags[ind_Nf]: # vai ter intâncias da Fanchirp
            for alphas_ind in range(1, len(initial_alphas)-1):
                if initial_alphas[alphas_ind] != 0:
                    STFChT= fanchirpogram(x, initial_alphas[alphas_ind], sr, n_fft=win_lengths[ind_Nf], hop_length=hop_length, win_length=win_lengths[ind_Nf], window='hann', center=center, pad_mode='reflect')
                    if center:
    ...
    for ind_alphas in range(TFRs_tensor.shape[2] -1):
        ...
        inds_matrix = np.logical_and(alphas > initial_alphas[ind_alphas] , alphas <= initial_alphas[ind_alphas + 1])
    ...
    for ind_C in range(C_limits.shape[1] -1):
        ...
        inds_matrix  = np.logical_and(C > C_limits[0, ind_C], C <= C_limits[0, ind_C  + 1])
    ...
    TFR_comb[C <= C_limits[0,0]] = STFT_tensor[:,:,0][C <= C_limits[0,0]]
    # Preenchendo TFR onde C > C_limits[-1]
    aux_matrix = np.sum(weights_tensor_alphas * TFRs_tensor[:,:,:,-1], axis=2)
    TFR_comb[C > C_limits[0, -1]] = aux_matrix[C > C_limits[0,-1]]
    # preenchendo TFR nos ataques
    TFR_comb[alphas < initial_alphas[0]] = STFT_tensor[:,:,0][alphas <= initial_alphas[0]]
    TFR_comb[alphas >= initial_alphas[-1]] = STFT_tensor[:,:,0][alphas >= initial_alphas[-1]]